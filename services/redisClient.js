...
export const getRedisClient = async () => {
  ...
  client = createClient({ url: REDIS_URL, password: process.env.REDIS_PASSWORD || undefined });
  connecting = true;
  client.on('error', (err) => {
  ...
  try {
    ...
    console.error('Failed to connect redis client:', err && err.message ? err.message : err);
  }
  connecting = false;
  return client;
};
