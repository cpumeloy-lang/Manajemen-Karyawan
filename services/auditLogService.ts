 ...
 */
export const deleteAllAuditLogs = async (): Promise<{ deletedCount: number }> => {
    try {
        ...
        if (!response.ok) {
            throw new Error(result?.error || 'Gagal menghapus audit log');
        }
...
/**
 * Clear audit logs older than X days (admin only)
 * Server-side authorization enforced
 */
export const clearOldAuditLogs = async (olderThanDays: number = 30): Promise<{ deletedCount: number }> => {
    try {
        const { data: sessionData } = await supabase.auth.getSession();
        const token = sessionData.session?.access_token;

        const response = await fetch(`/api/audit-logs?olderThanDays=${olderThanDays}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                ...(token ? { Authorization: `Bearer ${token}` } : {}),
        ...
        const result = await response.json().catch(() => null);
        if (!response.ok) {
            throw new Error(result?.error || 'Gagal membersihkan log lama');
        }
